...
check_ssl () {
   ...
   then
    ...
    for i in ${CERTS}
     ...
     kill -15 "${API_PID[@]}"
     wait "${API_PID[@]}"
     bashio::log.info "Start Dynu Reverse Proxy..."
...
if ! grep -q "certbot" /etc/crontabs/root
 then
   echo "0 12 * * * /usr/bin/certbot renew --quiet --config-dir /ssl/dynu/ ; nginx -s reload 2>&1 >> /dev/null" >> /etc/crontabs/root
fi
...
/usr/bin/noipy -u ${TOKEN} -n ${DOMAIN} --provider dynu
